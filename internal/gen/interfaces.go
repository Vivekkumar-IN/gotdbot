...
func generateInterfaces(classes map[string]*TLClass) {
	...
	if err := os.WriteFile("gen_interfaces.go", []byte(sb.String()), 0644); err != nil {
		log.Fatal(err)
	}
}

func generateTypes(types []TLType, classes map[string]*TLClass) {
	var sb strings.Builder
	sb.WriteString(header)
	sb.WriteString("package gotdbot\n\n")
	sb.WriteString("import \"encoding/json\"\n")
